<section class="bann02">
    <!-- <div class=""> -->
        <div class="myservices">
            <!-- <div class="princ-act"> -->
                <div class="vide01"></div>
                <div class="vide02"></div>

                <div class="service-card  border-me02 service04">
                    <div class="service-img">
                        <img src="<?php echo base_url('image/lit.jpg'); ?>" class="activit-img" alt="Pensionnat"> 
                    </div>
                    <div class="service-text">
                        <h3>Pensionnat</h3>
                        <p> Shalom - Maison d'Accueil est un lieu unique où nous prenons soin de nos résidents âgés dans un cadre stimulant, encourageant et personnalisé.</p>
                    </div>
                </div> 
                <div class="service-card  border-me02 service01">
                    <div class="service-img">
                        <img src="<?php echo base_url('image/cuisine02.jpg'); ?>" class="activit-img" alt="Nutri-thérapie">
                    </div>
                    <div class="service-text">
                        <h3>Nutri-thérapie</h3>
                        <p>En leur apprenant une alimentation adaptée par la Nutrithérapie (ou en prise en charge nutritionnelles pour les internats)</p>
                    </div>
                </div>
            <!-- </div> -->
            <!-- <div class="princ-act"> -->
                <div class="service-card  border-me02 service02">
                    <div class="service-img">
                        <img src="<?php echo base_url('image/gym.jpg'); ?>" class="activit-img" alt="Gymnastique douce">
                    </div>
                    <div class="service-text">
                        <h3>Gym douce</h3>
                        <p>En leur accordant plusieurs activités physiques comme la gymnastique douce favorisant un renforcement musculaire adéquat et le bon maintien de l’équilibre.</p>
                    </div>
                </div>
            <!-- </div> -->
            <!-- <div class="princ-act"> -->
                <div class="service-card  border-me02 service03">
                    <div class="service-img">
                        <img src="<?php echo base_url('image/atelier.jpg'); ?>" class="activit-img" alt="Divers ateliers"> 
                    </div>
                    <div class="service-text">
                        <h3>Ateliers et jeux</h3>
                        <p>Par la mise en place de différents jeux collectifs de stimulation et d’animations qui seront des moyens de rencontre et d’ouverture.</p>
                    </div>
                </div>   
            <!-- </div> -->
        </div>
    <!-- </div> -->
</section>